#!/bin/bash
#
# analyzeFD - Run all steps for analyzing afile download from Datafordeler
#
# The resulting files will be stored in a directory with the same basename as the inputfile
#
if [ $# -eq 0 ]
  then
    echo "No arguments supplied"
    echo "Usage"
    echo "   analyzeFD <zipfile>"
    echo "where"
...
SORTFILE=$RANDOM.tmp
if [[ -f ${CSVFILE}  ]]; then
   sort -T. -t, --key=5,5 --key=1,1 $CSVFILE > $SORTFILE
   mv $SORTFILE $CSVFILE
fi
#
